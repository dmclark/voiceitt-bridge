...
# @raycast.title Send to Supacode
# @raycast.mode silent
# @raycast.packageName Voiceitt
# @raycast.icon 🟪
# @raycast.description Cleanup at send: copy from focused app (Voiceitt textarea). When the page's AI toggle is ON (GET /ai-state == "1"), run through voiceitt-transform (fail-open to raw). When OFF, paste raw — no invisible LLM call. Then synthetic Cmd+V into Supacode. Sticky-Keys safe via cliclick. Does NOT press Return.

set -e
CLICLICK="/opt/homebrew/bin/cliclick"
TARGET_BUNDLE_ID="app.supabit.supacode"
...
if [ "$CURRENT" = "$SENTINEL" ] || [ -z "$CURRENT" ]; then
  osascript -e 'display notification "Cmd+A/Cmd+C did not capture text. Make sure the Voiceitt textarea is focused." with title "Send to Supacode"'
  exit 1
fi
