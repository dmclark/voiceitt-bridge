...
# @raycast.icon 🟪
# @raycast.description Send the scratchpad's outgoing text to Supacode. Reads /api/scratchpad-state instead of scraping the focused textarea, then synthetic Cmd+V into Supacode. Sticky-Keys safe via cliclick. Does NOT press Return.
...
if [ "$STATE_EXIT" -ne 0 ]; then
  osascript -e "display notification \"No outgoing scratchpad text found at $API_BASE. Open the scratchpad and dictate or edit text first.\" with title \"Send to Supacode\""
  exit 1
fi
