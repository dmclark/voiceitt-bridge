...
# @raycast.icon 🟪
# @raycast.description Send the v1 scratchpad's outgoing text to AMP. Reads /api/scratchpad-state instead of scraping the focused textarea, then synthetic Cmd+V into AMP. Sticky-Keys safe via cliclick. Does NOT press Return.

set -e
CLICLICK="/opt/homebrew/bin/cliclick"
TARGET_BUNDLE_ID="com.google.Chrome.app.bpgpkfmeagdpdcenbogglipmfkalmple"
...
# 1) Read the page-owned outgoing buffer from the v1 API. This replaces
...
if [ "$STATE_EXIT" -ne 0 ]; then
  osascript -e "display notification \"No outgoing scratchpad text found at $API_BASE. Open the v1 scratchpad and dictate or edit text first.\" with title \"Send to AMP\""
  exit 1
fi
