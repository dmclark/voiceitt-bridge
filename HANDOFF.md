...
>
> - **MVP — ships first, no Python *architecture*.** Cleanup runs at send-time via `raycast/voiceitt-transform` (bash + curl + jq, carried over verbatim from the prototype); the `send-to-*` Raycast hotkeys shell out to it and then drop the cleaned text into the target app via the sentinel + cliclick paste ritual (or, for iTerm, AppleScript `write text … newline NO`). The scratchpad page itself is served by `bridge/serve.py` — the prototype's ~315-line `ThreadingHTTPServer` stdlib script, also carried verbatim. **"No Python" here means no FastAPI, no `pyproject.toml`, no `src/voiceitt_bridge/` package, no `tests/` — i.e. no Python *architecture*.** A single stdlib script is allowed (and is the smallest thing that makes the repo standalone for net-new users without the prototype installed).
...
>
> Treat the rest of this document's Python-transform sections (FastAPI app, pluggable provider, prompt picker plumbing, framing module, tests) as the *post-MVP destination*, not MVP work. The four-endpoint contract itself (`/transform`, `/load`, `/file`, `/events`) is MVP — implemented in `bridge/serve.py`. See the 2026-05-22 entry in `salvage/notes/PARKING-LOT.md` for the sequencing note.
